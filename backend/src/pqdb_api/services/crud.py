...
def resolve_physical_column(
    ...
    *,
    ...
    System columns (id, created_at, updated_at) are rejected on write paths
    (for_insert=True) since they are auto-generated by the database.
    On read/filter paths (for_filter=True) they are treated as plain columns.
    """
    meta = _find_column_meta(col_name, columns_meta)
    if meta is None:
        raise CrudError(f"Unknown column: {col_name!r}")

    # Reject system columns on write paths (insert/update values)
    if for_insert and meta.get("is_system"):
        raise CrudError(
            f"Cannot write to system column {col_name!r}: "
...
def validate_columns_for_insert(
    ...
    System columns (id, created_at, updated_at) are rejected — they are
    auto-generated by the database and cannot be set by clients.
    """
    ...
    for key, value in row.items():
        ...
        # Reject system columns on insert — they are auto-managed
        if meta.get("is_system"):
            raise CrudError(
                f"Cannot insert into system column {key!r}: "
                "id, created_at, and updated_at are auto-generated"
            )
