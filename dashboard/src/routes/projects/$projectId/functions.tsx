  ...
  return (
    <AuthGuard>
      <ProjectProvider projectId={projectId}>
        <FunctionsRouteInner projectId={projectId} />
      </ProjectProvider>
    </AuthGuard>
  );
}

function FunctionsRouteInner({ projectId }: { projectId: string }) {
    ...
    return (
      <div className="text-center py-12">
        <p className="text-muted-foreground">
          No API key found. Create an API key to view functions.
        </p>
      </div>
    );
  }

  return <FunctionsPage projectId={projectId} apiKey={apiKey} />;
}
