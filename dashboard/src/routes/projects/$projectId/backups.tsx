  ...
  return (
    <AuthGuard>
      <ProjectProvider projectId={projectId}>
        <BackupsRouteInner projectId={projectId} />
      </ProjectProvider>
    </AuthGuard>
  );
}

function BackupsRouteInner({ projectId }: { projectId: string }) {
    ...
    return (
      <div className="text-center py-12">
        <p className="text-muted-foreground">
          No API key found. Create an API key to view backup stats.
        </p>
      </div>
    );
  }

  return <BackupsPage projectId={projectId} apiKey={apiKey} />;
}
